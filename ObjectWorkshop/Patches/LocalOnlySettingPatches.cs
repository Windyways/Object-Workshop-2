...
public static class LocalSettings
{
    ...
    [
        new()
        {
            Title = $"Button Scale Factor: {Math.Round(OWPlugin.ButtonUIFactor.Value, 2)}x",
            ...
            OnClick = () =>
            {
                ...
                {
                    HudManagerPatches.ResizeUI(1f / OWPlugin.ButtonUIFactor.Value);
                }

                var newVal = OWPlugin.ButtonUIFactor.Value + 0.1f;
                if (newVal is <= 0.5f or >= 1.6f)
                {
                    newVal = 0.5f;
                }

                OWPlugin.ButtonUIFactor.Value = newVal;
                if (HudManager.InstanceExists)
                {
                    HudManagerPatches.ResizeUI(OWPlugin.ButtonUIFactor.Value);
                }
                ...
                return OWPlugin.ButtonUIFactor.Value < 1f;
            },
            DefaultValue = OWPlugin.ButtonUIFactor.Value < 1f
        },
        new()
        {
            Title = "Offset Buttons If You Can't Vent",
            ObjName = "OffsetButtons",
            OnClick = () => { return OWPlugin.OffsetButtons.Value = !OWPlugin.OffsetButtons.Value; },
            DefaultValue = OWPlugin.OffsetButtons.Value
        },
        new()
        {
            Title = $"Role Icon Position: {OWAssets.RoleIconPosName}",
            ObjName = "IconPos",
            OnClick = () =>
            {
                var newVal = OWPlugin.RoleIconSpot.Value + 1;
                if (newVal is < 0 or > 2)
                {
                    newVal = 0;
                }

                OWPlugin.RoleIconSpot.Value = newVal;
                ...
                if (optionsMenu != null)
                {
                    ...
                    if (title != null && title.transform.GetChild(2).TryGetComponent<TextMeshPro>(out var txt))
                    {
                        txt.text = $"Role Icon Position: {OWAssets.RoleIconPosName}";
                    }
                }
