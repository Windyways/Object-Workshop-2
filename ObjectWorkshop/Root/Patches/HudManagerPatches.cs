...
public static class HudManagerPatches
{
    public static bool LocalVisibilityFlag(PlayerControl localPlayer, PlayerControl player)
    {
        return
            ...
            (localPlayer.Is(Faction.Infiltrator) && player.TryGetModifier<RoleLearn>(out var infiltratorRevealed) && infiltratorRevealed.Visitor.Is(Faction.Infiltrator)) ||
    ...
    public static bool VisibilityFlag(PlayerControl player)
    {
        ...
        /*return
            // Infiltrator
            (PlayerControl.LocalPlayer.Is(Faction.Infiltrator) && player.Is(Faction.Infiltrator)) ||
            (PlayerControl.LocalPlayer.Is(Faction.Infiltrator) && player.TryGetModifier<RoleLearn>(out var infiltratorRevealed) && infiltratorRevealed.Visitor.Is(Faction.Infiltrator)) ||
    ...
    public static IEnumerator CoResizeUI()
    {
        ...
        ResizeUI(OWPlugin.ButtonUIFactor.Value);
    }
    ...
    public static void ResizeUI(float scaleFactor)
    {
                     ...
                     .GetComponentsInChildren<GridArrange>(true))
        {
            ...
            if (arrange.isActiveAndEnabled && arrange.gameObject.transform.childCount != 0)
            {
                ...
                {
                    // Logger<OWPlugin>.Error($"Error arranging child objects in GridArrange: {e}");
                }
            }
        }
    }
    ...
    public static void UpdateRoleNameText()
    {
        ...
        if (MeetingHud.Instance)
        {
            foreach (var playerVA in MeetingHud.Instance.playerStates)
            {
                ...
                {
                    playerColor = RoleColors.Infiltrator;
                }
...
#pragma warning restore S1854 // Unused assignments should be removed
                    ...
                    revealMods.Any(x => x.Visible && x.RevealRole))
                {
                    ...
                    if (player.TryGetModifier<DeathHandlerModifier>(out var deathMod) && OptionGroupSingleton<OWOptions>.Instance.AliveSeeDeathReason)
                    {
        ...
        else
        {
            ...
            foreach (var player in PlayerControl.AllPlayerControls)
            {
                ...
                if (PlayerControl.LocalPlayer.Is(Faction.Infiltrator) && player.Is(Faction.Infiltrator) && PlayerControl.LocalPlayer != player)
                {
                    playerColor = RoleColors.Infiltrator;
                }
