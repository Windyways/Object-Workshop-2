...
public static class DeadSeeVoteColorsPatch
{
        ...
        [HarmonyArgument(1)] int index, [HarmonyArgument(2)] Transform parent)
    {
        ...
        if (Debugger.IsDebuggerActive && player != null)
        {
            ...
            else if (player.Is(Faction.Neutral)) PlayerMaterial.SetColors(RoleColors.Neutral, spriteRenderer);
            else if(player.Is(Faction.Infiltrator)) PlayerMaterial.SetColors(RoleColors.Infiltrator, spriteRenderer);
        }
