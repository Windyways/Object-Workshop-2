...
public static class EndGamePatches
{
    public static void BuildEndGameData()
    {
        ...
        foreach (var playerControl in PlayerControl.AllPlayerControls)
        {
                         ...
                         .Select(x => x.Value))
            {
                ...
                playerRoleString.Append(OWPlugin.Culture, $"{color.ToTextColor()}{roleName}</color> > ");
            }
            ...
            if (modifierCount != 0)
            {
                playerRoleString.Append(OWPlugin.Culture, $" (");
            }
            ...
            foreach (var modifierName in modifierNames)
            {
                ...
                if (modifierCount == 0)
                {
                    playerRoleString.Append(OWPlugin.Culture, $"{modColor.ToTextColor()}{modifierName}</color>)");
                }
                else
                {
                    playerRoleString.Append(OWPlugin.Culture,
            ...
            {
                playerRoleString.Append(OWPlugin.Culture, $" | {deathHandler.DeathColor.ToTextColor()}{deathHandler.CauseOfDeath.ToSpacedString()}</color>");
                if (deathHandler.SecondaryCauseOfDeath != DeathReasonShow.Alive)
                {
                    playerRoleString.Append(OWPlugin.Culture, $" | {deathHandler.SecondaryDeathColor.ToTextColor()}{deathHandler.SecondaryCauseOfDeath.ToSpacedString()}</color>");
                }
                if (deathHandler.ThirdCauseOfDeath != DeathReasonShow.Alive)
                {
                    playerRoleString.Append(OWPlugin.Culture, $" | {deathHandler.ThirdDeathColor.ToTextColor()}{deathHandler.ThirdCauseOfDeath.ToSpacedString()}</color>");
                }
            }
            else
            {
            ...
            {
                playerName.Append(OWPlugin.Culture, $"<color=#EFBF04>{playerControl.Data.PlayerName}</color>");
                playerWinner = true;
    ...
    public static void BuildEndGameSummary(EndGameManager instance)
    {
        ...
        foreach (var data in EndGameData.PlayerRecords)
        {
            ...
            if (count % 2 == 0)
            {
                roleSummaryText2.AppendLine(OWPlugin.Culture, $"{data.PlayerName} - {role}");
            }
            else
            {
                roleSummaryText1.AppendLine(OWPlugin.Culture, $"{data.PlayerName} - {role}");
            }

            count++;
            roleSummaryBackup.AppendLine(OWPlugin.Culture, $"{data.PlayerName} - {role}");
            roleSummaryTextFull.AppendLine(OWPlugin.Culture, $"{data.PlayerName} - {role}");
        }
    ...
    public static void AfterEndGameSetup(EndGameManager instance)
    {
        ...
        switch (EndGameEvents.winType)
        {
            case 1:
                ...
                break;
            case 2:
                text.text = "<size=4>Infiltrator Win!</size>";
                text.color = RoleColors.Infiltrator;
                instance.BackgroundBar.material.SetColor(ShaderID.Color, RoleColors.Infiltrator);
                break;
            default:
