...
public static class OW_ReportBodyEvent
{
    ...
    public static void ReportBodyEvent(ReportBodyEvent @event)
    {
        ...
        var gravekeeper = PlayerControl.AllPlayerControls.ToArray().Any(x => !x.HasDied() && x.Data.Role is Gravekeeper gravekeeper && gravekeeper.GravekeeperSeason());
        if (shikari || gravekeeper) @event.Cancel();
    }
}