...
public static class InstanceControlPatches
{
    ...
    public static class OnLobbyStart
    {
        ...
        {
            if (Debugger.IsDebuggerActive && OWPlugin.Persistence && Clients.Count != 0)
            {
    ...
    public static class AirshipSpawn
    {
        public static void Postfix(SpawnInMinigame __instance)
        {
            ...
            foreach (var player in PlayerControl.AllPlayerControls)
            {
                if (!OWPlugin.IsBot.Contains(player))
                {
                ...
                if (!player.Data.PlayerName.Contains(OWPlugin.RobotName))
                    continue;
